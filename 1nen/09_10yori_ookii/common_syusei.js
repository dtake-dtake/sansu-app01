...
function initializeDrillApp(config) {
    ...
    const KEY_HISTORY        = `${config.app.Id}:history`;
    ...
    function displayProblems(generatedProblems) {
      ...
      generatedProblems.forEach((p, i) => {
        ...
        // ★★★ ここを修正 ★★★
        // configのopが「と」の場合、特別な表示形式「A と B で」になるように変更
        let problemText;
        if (p.op === 'と') {
          problemText = `<span>${p.a}</span> と <span>${p.b}</span> で`;
        } else {
          problemText = `<span>${p.a}</span><span>${p.op}</span><span>${p.b}</span>=`;
        }
        eq.innerHTML = problemText;
        // ★★★ 修正ここまで ★★★
